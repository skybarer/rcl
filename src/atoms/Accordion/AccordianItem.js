...
const AccordionItem = ({ children }) => {
    ...
    const svgOpen = (
        <svg className='svg' fill="none" height="14" viewBox="0 0 9 14" width="9" xmlns="http://www.w3.org/2000/svg">
        ...
        <React.Fragment>
            {
                React.Children.map(children, (item, innerIndex) => {
                    if (item.type === AccordionTitle) {
                        ...
                        return <AccordionTitle titileOnClick={titileOnClick} toogleBody={toogleBody} svgShow={toogleBody ? svgClose : svgOpen}  {...item.props} />
